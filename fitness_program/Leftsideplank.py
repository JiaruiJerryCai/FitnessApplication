...
class set:
    ...
    def process(self, frame):
        ...
        try: 
            ...
            if self.backAlwaysStraight and self.kneeAlwaysStraight and self.headAlwaysStraight and self.leftArmAngleAlwaysWide and self.rightArmAlwaysUp and self.frame == None:
                self.frame = 1
            ...
            error_msg = "left arm not wide enough"
            if leftArmAngle < 70: 
                self.leftArmAngleAlwaysWide = False
                if error_msg not in self.error_dict:
                    self.error_dict[error_msg] = time.time()

            #Verify if right arm is raised up
            error_msg = "right arm not raised"
            if rightArmAngle < 80: 
                self.rightArmAlwaysUp = False
                if error_msg not in self.error_dict:
                    self.error_dict[error_msg] = time.time()
    ...
    def completed(self):
        # Increase the timer
        if self.backAlwaysStraight and self.kneeAlwaysStraight and self.headAlwaysStraight and self.leftArmAngleAlwaysWide and self.rightArmAlwaysUp and self.frame:
